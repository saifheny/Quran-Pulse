...
let isSurahsLoaded = false;
let isSurahsLoading = false;
async function fetchSurahs() {
if (isSurahsLoaded) return;
if (isSurahsLoading) {
while(isSurahsLoading) await new Promise(r => setTimeout(r, 100));
return;
}
...
async function playReciter(id) {
currentReciter = recitersData.find(r => r.id === id);
document.getElementById('persistent-player').style.display = 'block';
document.getElementById('p-title').innerText = "جاري التحميل...";
document.getElementById('p-reciter').innerText = currentReciter.name;
document.getElementById('playIcon').className = 'fas fa-circle-notch fa-spin';

await fetchSurahs();

if (!allSurahs || allSurahs.length === 0) {
alert("تعذر تحميل قائمة السور. يرجى التحقق من اتصال الإنترنت.");
